...
/**
 * 毛玻璃（glassmorphism）工具。
 *
 * 技术约束：真实的背景模糊依赖 [RenderEffect]，仅在 **API 31+** 可用，
 * 而本项目 minSdk 为 26。因此：
 *  - API 31+：调用 [applyGlassBlur] 得到真·磨砂效果；
 *  - API 26-30：[applyGlassBlur] 为 no-op，由 drawable 的「半透明填充 + 1dp 细描边」
 *    模拟毛玻璃层次，视觉上依然成立。
 *
 * 用法要点：模糊应作用于**装饰性背景层**（例如首页底图），而不是玻璃面板本身；
...
object GlassUi {
     ...
     */
    fun View.applyGlassBlur(radiusDp: Float = DEFAULT_BLUR_RADIUS_DP) {
